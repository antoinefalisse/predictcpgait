...
for i = 1:length(idx_Qs)
    ...
    for k = 1:length(ww)
        ...
        p(k) = plot(Qs_toTrack(ww(k)).m(mpi).mp(:,1),Qs_r(ww(k)).m(mpi).mp(:,idx_Qs(i)),...
            'color',color_all(k,:),'linestyle','-.','linewidth',line_linewidth);    
        hold on;        
...
% Analyse RMS
corrCPAll.mp.rmse.angles.diff = ...
    diff(corrCPAll.mp.rmse.angles.all,1,1);
...
for i = 1:length(idx_As)
    ...
    if EMGcol(idx_As(i)) ~= 99
        ...
        for j = 1:size(ref_EMG.(subject).all,3)
            emg_peak(j) = nanmax(ref_EMG.(subject).all(:,strcmp(ref_EMG.(subject).colheaders,['R',EMGchannel{idx_As(i)}]),j),[],1);
        end
        norm_f = a_peak./emg_peak;
        tempp(:,:) = ref_EMG.(subject).all(:,strcmp(ref_EMG.(subject).colheaders,['R',EMGchannel{idx_As(i)}]),:);
    ...
    for k = 1:length(ww)
        ...
        p(k) = plot(x,Acts_noSpas_r(ww(k)).m(mpi).mp(:,idx_As(i)+NMuscles/2),'color',...
            color_all(k,:),'linestyle','-.','linewidth',line_linewidth);
        hold on
        count = count+1;
...
for i = 1:4
    ...
    for k = 2
     ...
     % Plot settings
    set(gca,'Fontsize',label_fontsize)
    title(['4 synergies: ',num2str(pos_WSyn4i.right(i))],'Fontsize',label_fontsize);   
    % X-axis
    set(gca,'XTick',[]);
    ylim([0,1]);
    NumTicks = 2;
...
figure()
subplot(5,5,2:5)
b = bar(w_syn_resh.four.right(:,3),'stacked');
hold on
for j = 5:5:40
    plot([j j],[0 1],'k:','linewidth',1);
end
...
b.FaceColor = color_all(k,:);
b.EdgeColor = color_all(k,:);
size_x = length(w_syn_resh.four.right(:,3))+1;
Xt = 0:size_x;
Xl = [0 size_x];
...
% save('sumResults_generic','sumResults')        

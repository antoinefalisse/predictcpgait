...
for k = 1:length(ww) 
    ...
    for mpi = 1:NPhases(ww(k)).m
        ...
        Fpe_l(ww(k)).m(mpi).mp = ResultsPredSim.(['Case_',num2str(ww(k))]).Fpe_l(mpi).mp;
        lMtilde_l(ww(k)).m(mpi).mp = ResultsPredSim.(['Case_',num2str(ww(k))]).lMtilde_l(mpi).mp;
...
for i = 1:length(idx_As)
    ...
    for k = 1:length(ww)
        ...
        p(k) = plot(x,Acts_noSpas_l(ww(k)).m(mpi).mp(:,idx_As(i)).^10,'color',...
            color_all(k,:),'linewidth',line_linewidth);
        hold on
        count = count+1;
        plot([x(TO(mpi).l(k)) x(TO(mpi).l(k))],[0 4e-4],'color',color_all(k,:),'linewidth',1);
    end   
    count = 1;
    % Plot settings
    set(gca,'Fontsize',label_fontsize)
...
for k = 1:length(ww)
    NMuscle_noFLV_r(ww(k)).m  = length(muscleNames_noFLV(ww(k)).m); % one side
    ...
    musi_noFLV(ww(k)).m = [musi_noFLV_r(ww(k)).m,musi_noFLV_r(ww(k)).m]; % both sides    
    musi = 1:NMuscles;
    musi_FLV(ww(k)).m = musi;
    musi_FLV(ww(k)).m(musi_noFLV(ww(k)).m) = [];    
    for mpi = 1:NPhases(ww(k)).m    
        Fpe_l_adj(ww(k)).m(mpi).mp = zeros(size(Fpe_l(ww(k)).m(mpi).mp,1),NMuscles);
        Fpe_l_adj(ww(k)).m(mpi).mp(:,musi_FLV(ww(k)).m) = Fpe_l(ww(k)).m(mpi).mp;        
        lMtilde_l_adj(ww(k)).m(mpi).mp = zeros(size(lMtilde_l(ww(k)).m(mpi).mp,1),NMuscles);
        lMtilde_l_adj(ww(k)).m(mpi).mp(:,musi_FLV(ww(k)).m) = lMtilde_l(ww(k)).m(mpi).mp;
    end    
end
for mpi = 1:NPhases(ww(k)).m
pos_pF = [4,5];
...
for i = 1:length(idx_pF)
    ...
    for k = 1:length(ww)
        x = 1:(100-1)/(size(Acts_noSpas_l(ww(k)).m(mpi).mp,1)-1):100;            
        plot(x,Fpe_l_adj(ww(k)).m(mpi).mp(:,idx_pF(i))./MTParameters(1,idx_pF(i)),...
            'color',color_all(k,:),'linewidth',line_linewidth);
        hold on;
    ...
    set(gca,'YTick',linspace(LY(1),LY(2),NumTicks))
    if i == 1
        ylabel('Forces (-)','Fontsize',label_fontsize);
    end  
    box off;
end
end
